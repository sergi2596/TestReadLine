...
class EditableBufferedReader extends BufferedReader {
	...
	public String readLine() throws IOException {
		...
		while (cr != CTRLD) {
			...
			maxims();
			/*
			 * DEBUGGER PROPI: Obrir un altre terminal apart de la que utilitzem
			 * per executar el programa. Mirar amb la comana 'ps' quin es el tty
			 * del terminal i cambiar el /dev/pts/X de les comanes de sota. Aixi
			 * podem observar tots els paràmetres seguents en temps real:
			 * fila,columna,maxfil,maxcol,filainicial.
			 */
			/*try {
				String[] command = {
						"bash",
				...
				String[] command1 = {
						"bash",
						"-c",
						"echo maxfila:" + maxfil + " maxcol: " + maxcol
								+ " > /dev/pts/1" };
				InputStream in1 = Runtime.getRuntime().exec(command1)
						.getInputStream();
				String[] command2 = { "bash", "-c",
						"echo ----------------- > /dev/pts/1" };
				InputStream in2 = Runtime.getRuntime().exec(command2)
						.getInputStream();
			} catch (Exception e) {
				e.printStackTrace();
			}
*/
	...
	public boolean dinsText(int direccio) {
		...
		case DOWN_ARROW:
			...
			break;
		case RIGHT_ARROW:
			dins = ((fila == maxfil && columna == maxcol) || columna == TOTALCOLS) ? false
					: true;
			...
			break;
		case LEFT_ARROW:
			dins = ((fila == FILAINICIAL && columna == 1) || columna == 1) ? false
					: true;
		...
		try {
			...
			case 1:
				...
				if (longitud == 3) {
					...
					columna = Integer.parseInt(filacols.get(2) + ""
							+ filacols.get(3));
				}
				break;
			case 2:
				...
				if (longitud == 4) {
					...
					columna = Integer.parseInt(filacols.get(3) + ""
							+ filacols.get(4));
				}
				break;
			default:
